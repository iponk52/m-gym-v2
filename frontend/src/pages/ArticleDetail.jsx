...
export default function ArticleDetail() {
  ...
  const fetchData = async () => {
    try {
      const [artRes, homeRes] = await Promise.all([
        ...
        axios.get('http://localhost:3000/api/public/home')
      ]);
  ...
  if (!article) return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400">Artikel tidak ditemukan.</div>;
      ...
      <header className="bg-slate-900 text-white p-4 sticky top-0 z-40 shadow-lg">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <Link to="/" className="flex items-center gap-2 hover:text-blue-400 transition-colors">
            <ArrowLeft size={20} /> <span className="font-medium">Kembali ke Beranda</span>
          </Link>
          <div className="flex items-center gap-4">
            <button onClick={toggleDarkMode} className="text-yellow-400 hover:text-yellow-300 transition-colors p-2 bg-slate-800 rounded-full">
              {isDark ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            <div className="flex items-center gap-2 opacity-50">
              {settings?.logo_url ? (
                <img src={settings.logo_url} alt="Logo" className="w-6 h-6 object-contain" crossOrigin="anonymous" />
              ) : (
                <Dumbbell size={20} />
              )}
              <span className="font-bold tracking-wider">{settings?.name || 'M-GYM'}</span>
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="p-6 sm:p-12">
            ...
            <div className="flex flex-wrap items-center gap-6 mb-10 pb-6 border-b border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">{new Date(article.CreatedAt).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
              </div>
            </div>

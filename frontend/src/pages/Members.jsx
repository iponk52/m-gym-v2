...
export default function Members() {
  ...
  const validateDuplicate = async (field, value) => {
    ...
    try {
      const res = await axios.get(`http://localhost:3000/api/public/check-duplicate?field=${field}&value=${encodeURIComponent(value)}`);
      if (res.data.exists) {
        const errorMsgs = {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing && selectedMember) {
        await axios.put(`http://localhost:3000/api/members/admin/${selectedMember.ID}`, payload);
      } else {
        await axios.post('http://localhost:3000/api/members/register', payload);
      }
  ...
  const handleManualScan = async (qr_code) => {
    try {
      const res = await axios.post('http://localhost:3000/api/attendance/scan', { qr_code });
      fetchMembers();
    } catch (error) {
  ...
  const handleApprove = async (member) => {
    if (window.confirm(`Apakah member baru ${member.full_name} sudah bayar?`)) {
      try {
        const res = await axios.put(`http://localhost:3000/api/members/approve/${member.ID}`);
  ...
  const handleSendPassword = async (member) => {
    if (window.confirm(`Kirim password via WhatsApp ke ${member.full_name}?`)) {
      try {
        const res = await axios.post(`http://localhost:3000/api/members/send-password/${member.ID}`);
        if (res.data.text && res.data.phone) {
          let formattedPhone = res.data.phone.replace(/\D/g, '');
          if (formattedPhone.startsWith('0')) formattedPhone = '62' + formattedPhone.slice(1);
          const url = `https://wa.me/${formattedPhone}?text=${encodeURIComponent(res.data.text)}`;
  ...
  const handleDisable = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menonaktifkan (Disable) member ini? History pembayaran akan tetap tersimpan.')) {
      try {
        await axios.put(`http://localhost:3000/api/members/disable/${id}`);
  ...
  const handleRenew = async (member) => {
    if (window.confirm(`Apakah ${member.full_name} ingin melakukan perpanjangan (Renew) hari ini juga?`)) {
      try {
        const res = await axios.put(`http://localhost:3000/api/members/renew/${member.ID}`);
  ...
  const handleResetPassword = async (member) => {
    if (window.confirm(`Kirim link Reset Password via WhatsApp ke ${member.full_name}?`)) {
      try {
        const res = await axios.post(`http://localhost:3000/api/members/reset-password-link/${member.ID}`);
        if (res.data.text && res.data.phone) {
          let formattedPhone = res.data.phone.replace(/\D/g, '');
          if (formattedPhone.startsWith('0')) formattedPhone = '62' + formattedPhone.slice(1);
          const url = `https://wa.me/${formattedPhone}?text=${encodeURIComponent(res.data.text)}`;
                  ...
                  <tr key={m.ID} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 flex gap-2 justify-center items-center">
                      ...
                      <button onClick={async () => {
                        try {
                          const res = await axios.post(`http://localhost:3000/api/members/send-message/${m.ID}`);
                          let formattedPhone = res.data.phone.replace(/\D/g, '');

...
export default function Templates() {
  ...
  const [formData, setFormData] = useState({ title: '', content: '', type: 'umum', channel: 'wa' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ title: '', content: '', type: 'umum', channel: 'wa' });
      setEditingId(null);
      fetchTemplates();
    } catch (error) {
  ...
  const handleEdit = (t) => {
    setFormData({ title: t.title, content: t.content, type: t.type || 'umum', channel: t.channel || 'wa' });
    setEditingId(t.ID);
    setShowModal(true);
  };
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          onClick={() => { setFormData({ title: '', content: '', type: 'umum', channel: 'wa' }); setEditingId(null); setShowModal(true); }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800 flex items-center gap-2 flex-wrap">
                      ...
                      <span className={`px-2 py-0.5 text-xs rounded-full uppercase tracking-wider font-semibold ${t.channel === 'email' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 bg-white">
                  ...
                  <option value="reset">Reset Password (Kirim link reset via WA/Email)</option>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Saluran (Channel)</label>
                <select value={formData.channel || 'wa'} onChange={e => setFormData({ ...formData, channel: e.target.value })} className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 bg-white">
                  <option value="wa">💬 WhatsApp (WA)</option>
                  <option value="email">📧 Email SMTP</option>
                </select>
                <p className="text-xs text-slate-500 mt-2">Pilih saluran tujuan template ini dikirimkan.</p>
              </div>
              <div>

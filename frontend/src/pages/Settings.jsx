...
export default function Settings() {
  ...
  const handleUpdateText = async (e) => {
    ...
    try {
      const res = await axios.put('http://localhost:3000/api/settings', settings);
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:3000/api/settings/logo', formData, {
  ...
  const handleSaveMethod = async (e) => {
    ...
    try {
      if (editingMethod) {
        await axios.put(`http://localhost:3000/api/payment-methods/${editingMethod.ID}`, methodForm);
      } else {
        await axios.post('http://localhost:3000/api/payment-methods', methodForm);
      }
      fetchPaymentMethods();
  ...
  const handleDeleteMethod = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/api/payment-methods/${id}`);
      fetchPaymentMethods();
    } catch (error) {
  ...
  const handleSaveAdmin = async (e) => {
    ...
    try {
      if (editingAdmin) {
        await axios.put(`http://localhost:3000/api/admins/${editingAdmin.ID}`, adminForm);
      } else {
        await axios.post('http://localhost:3000/api/admins', adminForm);
      }
      fetchAdmins();
      setAdminModal(false);
    } catch (error) {

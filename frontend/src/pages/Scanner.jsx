...
export default function Scanner() {
  ...
  const fetchLinks = async () => {
    try {
      const res = await axios.get('http://localhost:3000/api/scanner-links');
  ...
  const handleCreateLink = async (e) => {
    ...
    try {
      await axios.post('http://localhost:3000/api/scanner-links', { name: newLinkName });
      setNewLinkName('');
      fetchLinks();
    } catch (error) {
  ...
  const handleDeleteLink = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/api/scanner-links/${id}`);
      fetchLinks();
    } catch (error) {

...
export default function Home() {
  ...
  const fetchPublicData = async () => {
    try {
      const res = await axios.get('http://localhost:3000/api/public/home');
      setData(res.data);
    } catch (error) {
  ...
  const validateDuplicate = async (field, value) => {
    ...
    try {
      const res = await axios.get(`http://localhost:3000/api/public/check-duplicate?field=${dbField}&value=${encodeURIComponent(value)}`);
      if (res.data.exists) {
        const errorMsgs = {
